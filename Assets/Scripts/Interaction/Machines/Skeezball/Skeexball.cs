        ...
        public override IEnumerable<ActorAction> GetActions(Actor context)
        {
            if (context == Actor)
            {
                ...
                else
                {
                    if (FireStrength <= 0)
                    {
                        yield return new ActorAction(eActionKey.FIRE, "Power Up", gameObject);
                    }
                    else
                    {
                        yield return new ActorAction(eActionKey.FIRE, "Fire", gameObject);
                    }
                }
                yield return new ActorAction(eActionKey.LOOK, "Aim Gun", gameObject);
            }
