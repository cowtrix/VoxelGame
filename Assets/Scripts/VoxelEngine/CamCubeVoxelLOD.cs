...
public class CamCubeVoxelLOD : VoxelLOD 
{
	...
	public override void Rebuild(VoxelRenderer renderer)
	{
		...
		try
		{
			...
			float bump = 1.5f;
			var left = GetTex(bounds.center + Vector3.left * bounds.extents.x * Scale * bump,
			...
			var right = GetTex(bounds.center + Vector3.right * bounds.extents.x * Scale * bump,
				rot * Quaternion.LookRotation(Vector3.left, up),
				bounds.size.zyx() * Scale, Vector2.one * Resolution);
			TexArray.SetPixels(right.GetPixels(), 1);
			right.SafeDestroy();

			var bottom = GetTex(bounds.center + Vector3.down * bounds.extents.y * Scale * bump,
			...
			var top = GetTex(bounds.center + Vector3.up * bounds.extents.y * Scale * bump,
				rot * Quaternion.LookRotation(Vector3.down, up),
				bounds.size.xzy() * Scale, Vector2.one * Resolution);
			TexArray.SetPixels(top.GetPixels(), 3);
			top.SafeDestroy();

			var back = GetTex(bounds.center + Vector3.back * bounds.extents.x * Scale * bump,
			...
			var front = GetTex(bounds.center + Vector3.forward * bounds.extents.x * Scale * bump,
